...
@views function ThermalConvection3D(;do_viz=true)
    ...
    for it = 1:nt
        dt = if it == 1
            ...
            min(5.0 * dt_diff, min(dx / max_g(abs.(Array(Vx))), dy / max_g(abs.(Array(Vy))), dz / max_g(abs.(Array(Vz)))) / 3.1)
        end
        @parallel assign!(T_old, T)
        errVx, errVy, errVz, errP, errT= 2 * ε, 2 * ε, 2 * ε, 2 * ε, 2 * ε
        iter = 1; niter = 0
        while (errVz > ε || errP > ε || errT > ε) && iter <= iterMax
            # @parallel assign!(ErrVx, Vx)
            # @parallel assign!(ErrVy, Vy)
            @parallel assign!(ErrVz, Vz)
            @parallel assign!(ErrP, Pt)
            @parallel assign!(ErrT, T)
            ...
            @parallel compute_Tflux!(dT_dt, T, T_old, Vx, Vy, Vz, dx, dy, dz, dt)
            @hide_communication b_width begin
                @parallel update_T!(T, dT_dt, qTx, qTy, qTz, dx, dy, dz, dτ_iter)
                @parallel (1:size(T, 1), 1:size(T, 2), 1:size(T, 3)) no_fluxZ_T!(T)
                update_halo!(T)
            end
            ...
            if mod(iter, nerr) == 0
                ...
                errT  = max_g(abs.(Array(ErrT)))  / (1e-12 + max_g(abs.(Array(T))))
